            ...
            <div className="fade has-bottom-nav hw-shell" style={{ width:"100%", maxWidth: mobile ? "100%" : 980 }}>
              <div style={{
                ...
                <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0, overflowY:"auto", background:"transparent" }}>
                  ...
                  {/* SCROLLABLE CONTENT */}
                  <div style={{ flex:1, display:"flex", flexDirection:"column", overflowY:"auto", padding:mobile?"0 0 100px":0 }}>

                    {/* ══════════ HERO BAND ══════════ */}
                    <div style={{
                      position:"relative", overflow:"hidden", flexShrink:0,
                      padding: mobile ? "32px 22px 28px" : "44px 52px 36px",
                      background:"linear-gradient(145deg,rgba(22,10,48,.95) 0%,rgba(12,5,26,.8) 100%)",
                      ...
                      {/* Gold ambient glow */}
                      <div style={{ position:"absolute", top:-60, left:-20, width:240, height:240, borderRadius:"50%", background:"radial-gradient(ellipse,rgba(201,168,76,.1) 0%,transparent 65%)", pointerEvents:"none" }}/>

                      <div style={{ position:"relative", display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:16 }}>
                        {/* Left: moon + greeting */}
                        <div style={{ display:"flex", alignItems:"center", gap:mobile?16:22, flex:1, minWidth:0 }}>
                          <div style={{ position:"relative", flexShrink:0 }}>
                            <div style={{ fontSize:mobile?56:72, lineHeight:1, filter:"drop-shadow(0 0 32px rgba(200,170,80,.7))", animation:"float 6s ease-in-out infinite", display:"block" }}>{moonPhase}</div>
                            {streak>=3 && <div style={{ position:"absolute", bottom:-4, right:-4, fontSize:14, filter:"drop-shadow(0 0 6px rgba(255,120,30,.8))" }}>🔥</div>}
                          </div>
                          <div style={{ minWidth:0 }}>
                            <div style={{ fontSize:10, letterSpacing:".18em", textTransform:"uppercase", color:"rgba(255,215,120,.45)", fontFamily:"'Nunito',sans-serif", fontWeight:700, marginBottom:7 }}>{greeting}</div>
                            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:mobile?"clamp(30px,9vw,42px)":48, fontWeight:800, letterSpacing:"-.035em", color:"#fff", lineHeight:.92, marginBottom:10 }}>
                              {active.child_name}
                            </div>
                            <div style={{ fontFamily:"'Crimson Pro',serif", fontStyle:"italic", fontSize:mobile?13:15, color:"rgba(255,255,255,.3)", lineHeight:1 }}>
                              {todayStory?"Tonight's story is waiting ✦":"What story shall we tell tonight?"}
                            </div>
                          </div>
                        </div>
                        ...
                        <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:6, flexShrink:0 }}>
                          <div style={{ display:"flex", gap:6 }}>
                            {[{v:streak||0,i:"🔥",glow:"rgba(255,120,30,.5)"},{v:childStories,i:"📖",glow:null},{v:badges.length,i:"🏅",glow:null}].map(({v,i,glow},idx)=>(
                              <div key={idx} style={{ display:"flex", alignItems:"center", gap:4, padding:"6px 10px", borderRadius:8, background:"rgba(255,255,255,.06)", border:"1px solid rgba(255,255,255,.07)", boxShadow: v>0&&glow?`0 0 12px ${glow}`:undefined }}>
                                <span style={{ fontSize:11 }}>{i}</span>
                                <span style={{ fontFamily:"'Nunito',sans-serif", fontSize:12, fontWeight:800, color: v>0?"rgba(255,255,255,.9)":"rgba(255,255,255,.3)" }}>{v}</span>
                              </div>
                            ))}
                          </div>
                          {!mobile && (
                            <button onClick={()=>setScreen("settings")} style={{ width:32, height:32, borderRadius:7, background:"rgba(255,255,255,.05)", border:"1px solid rgba(255,255,255,.07)", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer" }}>
                              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.35)" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-2 2 2 2 0 01-2-2v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83 0 2 2 0 010-2.83l.06-.06A1.65 1.65 0 004.68 15a1.65 1.65 0 00-1.51-1H3a2 2 0 01-2-2 2 2 0 012-2h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 010-2.83 2 2 0 012.83 0l.06.06A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 012-2 2 2 2 0 012 2v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 0 2 2 0 010 2.83l-.06.06A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 012 2 2 2 0 01-2 2h-.09a1.65 1.65 0 00-1.51 1z"/></svg>
                            </button>
                          )}
                        </div>
                      </div>
                    ...
                    {/* ══════════ CUSTOMIZE ══════════ */}
                    <div style={{ padding: mobile?"20px 20px 0":"28px 52px 0", display:"flex", flexDirection:"column", gap:20 }}>

                      {/* Story type — two big toggle cards side by side */}
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                        {[
                          {id:"adventure", label:"Adventure",  desc:"Pure imagination and fun", icon:"🌙"},
                          {id:"lesson",    label:"Life lesson", desc:"Woven with a gentle moral",  icon:"✨"},
                        ].map(t=>{
                          const on = storyMode===t.id;
                          return (
                            <button key={t.id} onClick={()=>setStoryMode(t.id)} style={{
                              padding: mobile?"14px 14px":"18px 18px",
                              borderRadius:14, cursor:"pointer", textAlign:"left",
                              border: on?"1.5px solid rgba(201,168,76,.6)":"1.5px solid rgba(255,255,255,.06)",
                              background: on
                                ? "linear-gradient(135deg,rgba(201,168,76,.12) 0%,rgba(201,168,76,.04) 100%)"
                                : "rgba(255,255,255,.04)",
                              transition:"all .18s",
                              boxShadow: on?"0 0 0 1px rgba(201,168,76,.1) inset,0 4px 20px rgba(201,168,76,.1)":undefined,
                              WebkitTapHighlightColor:"transparent",
                            }}>
                              <div style={{ fontSize:mobile?22:26, marginBottom:10 }}>{t.icon}</div>
                              <div style={{ fontFamily:"'Nunito',sans-serif", fontWeight:800, fontSize:mobile?13:14, color:on?"#f0d070":"rgba(255,255,255,.65)", marginBottom:4 }}>{t.label}</div>
                              <div style={{ fontFamily:"'Nunito',sans-serif", fontSize:mobile?10:11, color:on?"rgba(255,255,255,.4)":"rgba(255,255,255,.2)", lineHeight:1.45 }}>{t.desc}</div>
                            </button>
                          );
                        })}
                      </div>

                      {/* Lesson pills — only when lesson mode */}
                      {storyMode==="lesson" && (
                        <div>
                          <div style={{ fontSize:10, letterSpacing:".15em", textTransform:"uppercase", color:"rgba(255,255,255,.22)", fontFamily:"'Nunito',sans-serif", fontWeight:700, marginBottom:10 }}>Lesson</div>
                      ...
                      {/* Mood — scrollable row */}
                      <div>
                        <div style={{ fontSize:10, letterSpacing:".15em", textTransform:"uppercase", color:"rgba(255,255,255,.22)", fontFamily:"'Nunito',sans-serif", fontWeight:700, marginBottom:10 }}>Mood</div>
                        <div style={{ display:"flex", gap:8, overflowX:mobile?"auto":"visible", flexWrap:mobile?"nowrap":"wrap", scrollbarWidth:"none", paddingBottom:mobile?2:0 }}>
                          {MOODS.map(m=>(
                            <button key={m.id} onClick={()=>setMood(m.id)}
                              className={"sel-pill"+(mood===m.id?" on":"")}
                              style={{ flexShrink:0 }}>
                              {m.emoji} {m.label}
                            </button>
                          ))}
                      ...
                      {/* Last story — compact horizontal card */}
                      {lastStory && !todayStory && (
                        <div onClick={()=>{const ps=lastStory.text.split("\n\n✦\n\n");setPages(ps);setTitle(lastStory.title||"");setImgs(lastStory.page_images||[]);setCoverImg(lastStory.cover_image||null);setSpread(lastStory.cover_image?-1:0);setStory(lastStory);setStoryPhase("ready");setScreen("story");try{localStorage.setItem("dw_last_story",lastStory.id);}catch{}}}
                          style={{ display:"flex", alignItems:"center", gap:14, padding:"13px 16px", borderRadius:12, border:"1px solid rgba(255,255,255,.06)", background:"rgba(255,255,255,.03)", cursor:"pointer", transition:"background .15s" }}
                          onMouseEnter={e=>{e.currentTarget.style.background="rgba(255,255,255,.06)";}}
                          onMouseLeave={e=>{e.currentTarget.style.background="rgba(255,255,255,.03)";}}>
                          <div style={{ width:38, height:50, borderRadius:6, overflow:"hidden", flexShrink:0, background:"rgba(255,255,255,.06)" }}>
                            {lastStory.cover_image && <img src={lastStory.cover_image} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }}/>}
                          </div>
                          <div style={{ flex:1, minWidth:0 }}>
                            <div style={{ fontSize:10, letterSpacing:".1em", textTransform:"uppercase", color:"rgba(255,255,255,.25)", fontFamily:"'Nunito',sans-serif", fontWeight:700, marginBottom:3 }}>Last night</div>
                            <div style={{ fontFamily:"'Crimson Pro',serif", fontStyle:"italic", fontSize:14, color:"rgba(255,255,255,.75)", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{lastStory.title||"Your last story"}</div>
                          </div>
                          <span style={{ fontSize:10, color:"rgba(255,255,255,.2)", fontFamily:"'Nunito',sans-serif", flexShrink:0 }}>Re-read →</span>
                        </div>
                      )}
                      ...
                      {!active.character_card && (
                        <div onClick={()=>{setEditId(active.id);setPf(active);setScreen("profile");}}
                          style={{ display:"flex", alignItems:"center", gap:12, padding:"11px 14px", borderRadius:10, border:"1px dashed rgba(255,255,255,.08)", background:"rgba(255,255,255,.02)", cursor:"pointer", transition:"background .15s" }}
                          onMouseEnter={e=>{e.currentTarget.style.background="rgba(255,255,255,.05)";}}
                          onMouseLeave={e=>{e.currentTarget.style.background="rgba(255,255,255,.02)";}}>
                          <div style={{ width:28, height:28, borderRadius:7, border:"1px dashed rgba(255,255,255,.18)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.3)" strokeWidth="1.8" strokeLinecap="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg>
                          </div>
                          <span style={{ fontFamily:"'Nunito',sans-serif", fontSize:12, color:"rgba(255,255,255,.35)" }}>
                            📸 Add {active.child_name}'s photo — <span style={{ color:"rgba(255,255,255,.2)" }}>the hero will look just like them</span>
                          </span>
                        </div>
                      )}
                    ...
                    {/* ══════════ GENERATE BUTTON ══════════ */}
                    <div style={{ padding: mobile?"20px 20px 4px":"24px 52px 28px", marginTop:"auto" }}>
                      <button onClick={generateStory} style={{
                        width:"100%",
                        padding: mobile?"20px":"22px 32px",
                        borderRadius:18, cursor:"pointer",
                        position:"relative", overflow:"hidden",
                        background: todayStory
                          ? "rgba(255,255,255,.06)"
                          : "linear-gradient(135deg,#e8c84a 0%,#d4a030 40%,#b87a18 100%)",
                        color: todayStory ? "rgba(255,255,255,.45)" : "#1a0c00",
                        fontFamily:"'Nunito',sans-serif", fontWeight:900,
                        fontSize:mobile?17:18, letterSpacing:".01em",
                        border: todayStory?"1px solid rgba(255,255,255,.08)":"none",
                        boxShadow: todayStory ? "none" : "0 8px 40px rgba(200,140,20,.45), 0 1px 0 rgba(255,255,255,.15) inset",
                        transition:"all .2s cubic-bezier(.34,1.4,.64,1)",
                        WebkitTapHighlightColor:"transparent",
                      }}
                      onMouseEnter={e=>{ if(!todayStory){ e.currentTarget.style.transform="translateY(-3px) scale(1.01)"; e.currentTarget.style.boxShadow="0 16px 50px rgba(200,140,20,.6), 0 1px 0 rgba(255,255,255,.15) inset"; }}}
                      onMouseLeave={e=>{ e.currentTarget.style.transform=""; e.currentTarget.style.boxShadow=todayStory?"none":"0 8px 40px rgba(200,140,20,.45), 0 1px 0 rgba(255,255,255,.15) inset"; }}>
                        {/* Shimmer on active btn */}
                        {!todayStory && <div style={{ position:"absolute", inset:0, background:"linear-gradient(105deg,transparent 40%,rgba(255,255,255,.12) 50%,transparent 60%)", animation:"shimmer 3s ease-in-out infinite", pointerEvents:"none" }}/>}
                      ...
                      {!todayStory && (
                        <div style={{ textAlign:"center", marginTop:10, fontSize:11, color:"rgba(255,255,255,.18)", fontFamily:"'Nunito',sans-serif", letterSpacing:".08em" }}>
                          14 ILLUSTRATED PAGES · ~40 SECONDS
                        </div>
                      )}
                      <div style={{ display:"flex", justifyContent:"space-between", marginTop:14 }}>
                        <button onClick={()=>{setEditId(active.id);setPf(active);setScreen("profile");}} style={{ background:"none", border:"none", cursor:"pointer", fontFamily:"'Nunito',sans-serif", fontSize:11, color:"rgba(255,255,255,.18)", padding:0, transition:"color .14s" }} onMouseEnter={e=>e.currentTarget.style.color="rgba(255,255,255,.45)"} onMouseLeave={e=>e.currentTarget.style.color="rgba(255,255,255,.18)"}>Edit {active.child_name}'s profile →</button>
                        {!mobile && <div style={{ display:"flex", gap:18 }}>{[{l:"Library",fn:()=>{loadLibrary();setScreen("library");}},{l:"Badges",fn:()=>setScreen("badges")}].map(({l,fn})=>(<button key={l} onClick={fn} style={{ background:"none", border:"none", cursor:"pointer", fontFamily:"'Nunito',sans-serif", fontSize:11, color:"rgba(255,255,255,.18)", padding:0, transition:"color .14s" }} onMouseEnter={e=>e.currentTarget.style.color="rgba(255,255,255,.45)"} onMouseLeave={e=>e.currentTarget.style.color="rgba(255,255,255,.18)"}>{l}</button>))}</div>}
                      </div>
                    </div>
